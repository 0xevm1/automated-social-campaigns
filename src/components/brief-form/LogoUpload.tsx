...
import { uploadProductLogo } from '@/lib/api';
...
export function LogoUpload({ slug, onUploadComplete }: LogoUploadProps) {
  ...
  const doUpload = useCallback(async (file: File, targetSlug: string) => {
    ...
    try {
      const result = await uploadProductLogo(targetSlug, file);
      setState('uploaded');
  ...
  if (state === 'uploaded' && preview) {
    return (
      <div className="relative rounded-md border border-border overflow-hidden inline-block">
        <img
          src={preview}
  ...
  return (
    <div>
      ...
      >
        ...
        {state === 'uploading' ? (
          <div className="flex items-center justify-center gap-2 text-muted-foreground py-0.5">
          ...
          <div className="flex items-center justify-center gap-2 text-muted-foreground py-0.5">
            ...
            <span className="text-xs">Drop a logo or click to browse</span>
          </div>
        )}
      </div>
